...
static void switch_res_crt(unsigned width, unsigned height)
{
 ...
 //  {
      video_display_server_switch_resolution(width, height,
            ra_set_core_hz, ra_core_hz, crt_index);
      #if defined(__arm__)
 ...
 // pixel_clock = ((hmax*2) * vmax * hz) ;
  ...
  //    sprintf(set_hdmi, "hdmi_timings 1920 1 106 169 480 240 1 1 3 5 0 0 0 60 0 41458500 %d ", 1);
	//  sprintf(set_hdmi_timing, "hdmi_timings %d 1 %d %d %d %d 1 %d %d %d 0 0 0 %d 0 %f 1 ", width, hfp, hsp, hbp, height, vfp, vsp, vbp, hz, pixel_clock); 
	  	  sprintf(set_hdmi_timing, "hdmi_timings %d 1 %d %d %d %d 1 %d %d %d 0 0 0 %f %d %f 1 ", width, hfp, hsp, hbp, height, vfp,vsp, vbp, hz, ip_flag, pixel_clock); 
